...
agent_communication:
    ...
    - agent: "main"
      message: "🛒 ĐÃ HOÀN THÀNH BACKEND API CHO GIỎ HÀNG VÀ ĐƠN HÀNG! Thực hiện: 1) SHOPPING CART SYSTEM: CartItem và Cart models với session-based tracking, full CRUD operations (get, add, update, remove, clear cart), smart inventory checking, auto-calculate totals. 2) ORDER MANAGEMENT: CustomerInfo, OrderItem, Order models với comprehensive order tracking, auto-generate order numbers (KTH{date}{unique_id}), support COD và bank transfer payment methods, auto-calculate shipping fees. 3) BUSINESS LOGIC: Stock validation, size option handling, cart-to-order conversion, automatic cart clearing post-order. 4) ERROR HANDLING: Comprehensive error handling cho all edge cases. Ready để test backend trước khi implement frontend!"
    - agent: "testing"
